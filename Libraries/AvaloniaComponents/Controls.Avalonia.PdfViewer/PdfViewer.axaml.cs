...
{
    public static readonly DirectProperty<PdfViewer, byte[]?> SourceProperty =
        AvaloniaProperty.RegisterDirect<PdfViewer, byte[]?>(nameof(Source), o => o.Source, (o, v) => o.Source = v);
    ...
    private void Load(AvaloniaPropertyChangedEventArgs args)
    {
        try
        {
            ...
            var source = args.NewValue as byte[];
            if (source is null) return;

            _fileStream = new MemoryStream(source);
